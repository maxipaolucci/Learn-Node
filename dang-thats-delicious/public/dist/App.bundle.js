...
var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); //for autocomplete ajax calls
...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    //create bounds to then zoom the map until all our markers fit in it.
    ...
    //when clicks on the marker show the details of the place
...
//prevent hacks like onload in images. See final of course class 32

function searchResultsHTML(stores) {
  return stores.map(function (store) {
    return '\n      <a href="/store/' + store.slug + '" class="search__result">\n        <strong>' + store.name + '</strong>\n      </a>\n    ';
  }).join('');
} //for autocomplete ajax calls
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(res.data)); //sanitize what we insert in our html comming from db to prevent hacking like adding js in title names
        return;
      }
...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action) //"this" is the form
  .then(function (res) {
